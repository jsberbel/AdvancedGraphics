...
Gun::Gun(const std::string &name, int fireRate, int bulletPerShot, float spread, float bulletSpeed, int bulletDamage, const SerraEngine::SoundEffect & shootEffect) :
	m_name(name),
	m_fireRate(fireRate),
...
void Gun::fire(const glm::vec2 & position, const glm::vec2 & direction, std::vector<Bullet>& bullets) {
	...
	for (auto i = 0; i < m_bulletsPerShot; i++) {
		auto dir = glm::rotate(direction, glm::radians(randRot(randomEngine)));
